...
<div id="mytask-layout" class="theme-indigo">
	...
	<div class="main px-lg-4 px-md-4">
						...
						<div
							...
							<div class="col-auto d-flex w-sm-100">
								<button type="button" class="btn btn-dark btn-set-task w-sm-100" data-bs-toggle="modal" data-bs-target="#tickadd">
									<i class="icofont-plus-circle me-2 fs-6"></i> 이슈 추가
								</button>
							</div>
						</div>
					</div>
				...
				<div
					...
					<div class="modal-content">
						...
						<div class="modal-body">
							<div class="mb-3">
								<label for="issueTitle" class="form-label">이슈제목</label> <input
									type="text" class="form-control" id="issueTitle"
									placeholder="제목입력" />
							</div>
							<div class="mb-3">
								<label for="issueContent" class="form-label">이슈내용</label>
								<textarea class="form-control" id="issueContent" rows="3"
							...
							<div class="deadline-form">
								<div class="row g-3 mb-3">
									<div class="col">
										<label class="form-label">이슈상태</label> <select
											id="issueCodeList" class="form-select " multiple
											aria-label="Default select Priority">

										</select>
									</div>
									<div class="col">
										<label class="form-label">이슈카테고리</label> <select
											id="trackerCodeList" class="form-select " multiple
											aria-label="Default select Priority">

										</select>
									</div>
									<div class="col-sm-12">
										<label for="formFileMultipleone" class="form-label">이슈
											알림 대상자</label> <select id="alarmReceiver" class="form-select"
							...
							<div class="deadline-form">
								<div class="row g-3 mb-3">
									<div class="col">
										<label for="issueEndDate" class="form-label">예상 종료날짜</label> <input
											type="date" class="form-control" id="issueEndDate">
									</div>
								</div>
							</div>
						</div>
						<div class="modal-footer">
							 <button type="button" id="autoBtn" class="btn" style="color:gray;">자동완성</button>
		...
		<div class="modal fade" id="issueDetail" tabindex="-1"
			aria-hidden="true">
			<div
				class="modal-dialog modal-dialog-centered modal-md modal-dialog-scrollable">
				...
				<div class="modal-content">
					...
					<div class="modal-body">
						<div class="mb-3">
							<label class="form-label">이슈작성자</label> <input type="text"
								class="form-control" id="basicIssueWriter" readonly="readonly" />
						</div>
						<div class="mb-3">
							<label class="form-label">이슈제목</label> <input type="text"
								class="form-control" id="basicIssueTitle" readonly="readonly" />
						</div>
						<div class="mb-3">
							<label class="form-label">이슈내용</label>
							<textarea class="form-control" id="basicIssueContent" rows="3"
								readonly="readonly"></textarea>
						</div>
						<div class="deadline-form">
							<div class="row g-3 mb-3">
								<div class="col">
									<label class="form-label">이슈상태</label> <input type="text"
										class="form-control" id="basicIssueStatus" readonly="readonly"
										value="">
								</div>
								<div class="col">
									<label class="form-label">이슈카테고리</label> <input type="text"
										class="form-control" id="basicIssueTracker"
										readonly="readonly" value="">
								</div>
							</div>
							<div class="row g-3 mb-3">
								<div class="col">
									<label for="issueEndDate" class="form-label">이슈 발생날짜</label> <input
										type="date" class="form-control" id="basicIssueEventDate"
										readonly="readonly" value=""> <label
										for="issueEndDate" class="form-label">예상 종료날짜</label> <input
										type="date" class="form-control" id="basicIssueEndDate"
										readonly="readonly" value="">
								</div>
							</div>
						</div>
						<div class="mb-3">
							<label class="form-label">진행상태</label> <input type="text"
								class="form-control" id="basicIssueProcess" readonly="readonly" />
						</div>
						<div class="mb-3">
							<label class="form-label" id="commentLabel">이슈해결코멘트</label> <input
								type="text" class="form-control" id="basicIssueComment"
								readonly="readonly" />
						</div>
					</div>
				</div>
...
<script type="text/javascript">
				...
				.then(resp => {
					...
					proList.innerHTML+= data.map(item =>
						...
						<tr data-projectissueid =\${item.projectIssueId} data-issuetitle="\${item.issueTitle}" data-issuewriter=\${item.projectIssueWriter} data-issueeventdate =	\${item.issueEventDate} data-issuecomment = "\${item.projectIssueComment}"
							...
							<td>
							<input type="hidden" value="\${item.issueContent}" class="issueContentHidden">
							...
							<td>
								<img class="avatar rounded-circle" src="\${item.imageContents != null ? item.imageContents : '${cPath}/resources/images/xs/마스코트_투명.png'}" alt="">
							...
							<td>
							 ...
							 <td>
								...
								\${item.projectStatusId===1 && item.myRoleId===1 ? `<button data-projectissueiddata =\${item.projectIssueId} type="button" class = "btn btn-outline-success issueCheckBtn" >체크</button>` : `<span></span>`}
								 </div>
							 </td>
						 </tr>
	                              ...
	                              <div class="task-info d-flex align-items-center justify-content-between">
		                              <h6 class="light-info-bg py-1 px-2 rounded-1 d-inline-block fw-bold small-14 mb-0">제목: \${user.issueTitle}</h6>
			                              <div class="task-priority d-flex flex-column align-items-center justify-content-center">
			                                  ...
			                                  <img class="avatar rounded-circle" src="\${user.imageContents != null ? user.imageContents : '${cPath}/resources/images/xs/마스코트_투명.png'}" alt="">
			                                  </div>
			...
			params: {
				...
				.then(resp =>{
					...
					alarmReceiver.innerHTML+= data.map(item =>
							`
					<option selected value="\${item.memberId}" label="\${item.memberName}" >\${item.memberName}</option>
				` 
					).reduce(add)
				})
		}	
	...
	function addIssue()  {
		...
		insertForm.addEventListener('submit', function(e){
			...
			let projectiddata = document.querySelector('#projectIdHidden');
			...
			console.log("isenddatedata: ", isenddatedata);
			...
			const alarmReceiver = document.getElementById("alarmReceiver"); // 이슈 알림 대상자 <select> 요소
			const selectedOptions = alarmReceiver.selectedOptions; // 선택된 <option> 요소들의 유사 배열 객체

			const selectedValues = [];
			Array.prototype.forEach.call(selectedOptions, (option) => {
			  selectedValues.push(option.value); // 선택된 <option> 요소의 값을 배열에 추가
			  selectedValues.push(option.label); // 선택된 <option> 요소의 값을 배열에 추가
			});

			console.log("selectedValues", selectedValues); // 선택된 값들이 저장된 배열 출력
			...
			var selectedObj = $("#alarmReceiver").find("option:selected");

			$(selectedObj).each(function(index, obj){
			       var data = {
			    		   
			    		receiverId: $(obj).val(),
						alarmReceiver: $(obj).text()
						}
			    alarms.push(data); 
			});
			
			createIssueData.alarms = alarms;
			...
			console.log("마지막createIssueData: ", createIssueData);
			headers['Content-Type'] = 'application/json'
			headers['accept'] = 'application/json'
			axios.post(`/ourmine/company/\${companyIdHiddenValue}/project/\${projectIdHiddenValue}/projectIssue/projectIssueCreate`,createIssueData,{headers : headers})
					.then(resp => {
						let data = resp.data
						console.log("createIssueData", data)
						clearInsertModal();
						$("#tickadd").modal("hide");
						clearIssueList();
						issueList();
				...
				proList.innerHTML+= data.map(item =>
						`
							...
							<td>
							 ...
							 <td>
								 <div class="btn-group" role="group" aria-label="Basic outlined example">
								 \${item.projectStatusId===1 && item.projectRoleId===1 ? `<button data-projectissueiddata =\${item.projectIssueId} type="button" class = "btn btn-outline-success issueCheckBtn" >체크</button>` : `<span></span>`}
								 </div>
							 </td>
						 </tr>
...
// 			let issueContent = $(this).find(".issueContentHidden").val();
...
//         input: 'text',
        ...
        inputOptions: {
        	'코드 수정 완료했습니다.': '코드 수정 완료했습니다.',
        	'일정 변경 완료했습니다.': '일정 변경 완료했습니다.'
        },
        showCancelButton: true,
...
$('#autoBtn').on('click',function(){
	console.log("자동완성클릭");
$("#issueTitle").val("프로젝트 종료 알림");
$("#issueContent").val("그 동안 고생 많았습니다. 프로젝트가 종료됨을 알립니다.");
document.getElementById("issueCodeList").value = 3;
document.getElementById("trackerCodeList").value = 3;
document.getElementById("issueEndDate").value = getCurrentDate();
})
