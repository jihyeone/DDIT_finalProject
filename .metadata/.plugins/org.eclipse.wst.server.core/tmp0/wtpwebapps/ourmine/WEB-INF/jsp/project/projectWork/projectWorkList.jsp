    ...
    <div class="main px-lg-4 px-md-4">
					...
					<div class="border-0 mb-4">
						<div
							...
							<div class="col-auto d-flex w-sm-100">
								...
								<button id="taskAddBtn" type="button" class="btn btn-dark btn-set-task w-sm-100" data-bs-toggle="modal" data-bs-target="#pwInsertModal" style="display:none;">
									<i class="icofont-plus-circle me-2 fs-6"></i> 일감 추가
								</button>
								</security:authorize>
							</div>
						</div>
		...
		<div class="modal fade" id="pwInsertModal" tabindex="-1"  aria-hidden="true">
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<form id="pwInsertForm" novalidate>
						<div class="mb-3">
							...
							<input type="text" class="form-control" id="pwInProjectWorkTitle" name="projectWorkTitle" required="required">
						</div>
						<div class="mb-3">
								<i class="icofont-clock-time"></i> <label  class="form-label">상 태</label>
						...
						<div class="mb-3">
							<i class="icofont-user"></i> <label  class="form-label">담당자</label>
							<select ID= "getProjectMemberList" class="form-select " name="picId"></select>
				...
				<div class="modal-footer">
					<button type="button" class="btn" style="color:gray;" onclick="pwAutoBtn()">자동완성</button>
...
//<!-- ===================================================================================================================================== -->
    ...
    // input 필드의 값을 오늘 날짜로 설정
    
    const pwAutoBtn = () => {
    	console.log("실행되나");
    	let pwInProjectWorkTitle = document.querySelector("#pwInProjectWorkTitle");
    	let pwInProjectWorkStatusId = document.querySelector("#pwInProjectWorkStatusId");
  		
   	    document.getElementById("pwInProjectWorkStartDate").value = getCurrentDate();
   	    document.getElementById("pwInProjectWorkEndDate").value = getCurrentDate();
   	    document.getElementById("getProjectMemberList").value = 85;
   	    document.getElementById("pwInProjectWorkContent").value = "코드게시판 수정하기";
    	pwInProjectWorkTitle.value = "코드게시판 수정";
		...
		axios.get(`/ourmine/company/${companyId}/project/${projectId}/projectWork/projectWorkListJson`,{headers : headers})
				.then(resp => {
					...
					data-projectWorkEndDate =\${item.projectWorkEndDate} data-projectWorkWriteDate =\${item.projectWorkWriteDate} data-projectworkcontent ="\${item.projectWorkContent}"  data-companyid =\${item.companyId}  data-projectid =\${item.projectId} data-memberid =\${item.memberId} class="nowPwList">
							...
							<input type="hidden" data-projectworktitle ="\${item.projectWorkTitle}" name="projectWorkTitle1" class="projectWorkTitle1">
							<input type="hidden" value="\${item.projectWorkContent}" class="projectWorkContent1">
							<input type="hidden" value="\${item.projectWorkWriteDate}" class="projectWorkWriteDate">
							<td>
							...
							<td>
								<img class="avatar rounded-circle small-avt" src="\${item.imageContents !== null ? item.imageContents : '/ourmine/resources/mytask/documentation/assets/images/profile_av.png'}" alt="">
	                          ...
	                          <div class="dd-handle">
	                              <div class="task-info d-flex align-items-center justify-content-between">
	                                  ...
	                                  <div class="task-priority d-flex flex-column align-items-center justify-content-center">
	                                      ...
	                                      <div class="avatar-list avatar-list-stacked m-0">
	                                      	<img class="avatar rounded-circle small-avt" src="\${user.imageContents !== null ? user.imageContents : '/ourmine/resources/mytask/documentation/assets/images/profile_av.png'}" alt="">
	                                      </div>
	                                  </div>
	                              </div>
	                              <p class="py-2 mb-0">\${user.projectWorkContent}</p>
					...
					pageLinks.forEach((link, index) => {
						link.addEventListener('click', function(event){
							...
							.then(resp => {
								...
								pwList.innerHTML+= data.map(item =>
								`
								<input type="hidden" value="\${item.projectWorkTitle}"  class="projectWorkTitle2">
								<input type="hidden" value="\${item.projectWorkContent}" class="projectWorkContent1">
								<input type="hidden" value="\${item.projectWorkWriteDate}" class="projectWorkWriteDate">
...
//<!-- =============== 테이블 행을 클릭했을 때  ================================= -->
	...
	$(document).on('click','.nowPwList',function(){
		...
		//let projectWorkTitle = $(this).find(".projectWorkTitle1").val();
		//let projectWorkTitle = $(this).find(".projectworktitle").val();
		...
		//let projectWorkContent = $(this).find(".projectWorkContent1").val();
			...
			axios.get(`/ourmine/company/${companyId}/project/${projectId}/projectWork/projectWorkListJson`, {
				...
				pwList.innerHTML+= data.map(item =>
					...
					data-projectWorkEndDate =\${item.projectWorkEndDate} data-projectWorkWriteDate =\${item.projectWorkWriteDate} data-projectworkcontent ="\${item.projectWorkContent}"  data-companyid =\${item.companyId}  data-projectid =\${item.projectId} class="nowPwList">
						...
						<input type="hidden" value="\${item.projectWorkTitle}"  name="projectWorkTitle1" class="projectWorkTitle1">
						<input type="hidden" value="\${item.projectWorkContent}" class="projectWorkContent1">
						<input type="hidden" value="\${item.projectWorkWriteDate}" class="projectWorkWriteDate">
						<td>
							<a class="fw-bold text-secondary"  >\${item.projectWorkId}</a>
