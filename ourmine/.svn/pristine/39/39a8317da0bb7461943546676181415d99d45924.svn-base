...
<style>
	...
	#errorsContainer, #errorsContainer2{
		font-size : 1.3em;
	}
	#warningsContainer, #warningsContainer2{
		font-size : 1.3em;
	}
	#statsContainer, #statsContainer2{
		font-size : 1.3em;
	}
...
<!-- ===================================================================================================================================== -->
<!-- 상세조회 모달 -->
<div class="modal" id="selectCodeModal" style="background: rgba(0, 0, 0, 0.5);" >
			...
			<div class="card-body">
				<div class="row g-3 align-items-center">
					...
					<div class="col-md-12">
						<div class="form-group">
							<label>제목</label>
							<input type="text" id="codeTitle" name="codeTitle" class="form-control" disabled>
						</div>
					</div>
					<div class="col-md-12">
						<div class="form-group">
					...
					<div class="col-md-12">
						<div class="form-group">
							<label>내용</label><br>
							<textarea id="codeContent" name="codeContent" class="form-control code-textarea" rows="15" cols="300" disabled></textarea>
						</div>
					</div>
				</div>
...
<form id="insertCodeForm" novalidate method="post"> <!-- novalidate -->
	<div class="modal" id="insertModal" style="background: rgba(0, 0, 0, 0.5);">
						...
						<div class="col-md-12">
							<div class="form-group">
								...
								<select id="insertLanguageChoice" name="languageChoice" class="form-control" required onchange = "changeLanguage()">
							        <option value="4" active data-mime-type="text/x-java">Java</option>
							        <option value="5" data-mime-type="text/x-python">Python</option>
							        <option value="1" data-mime-type="text/x-csharp">C#</option>
							        <option value="8" data-mime-type="text/x-php">PHP</option>
							        <option value="17" data-mime-type="text/javascript">JavaScript</option>
							        <option value="28" data-mime-type="text/x-c++src">C++ (vc++)</option>
							        <option value="37" data-mime-type="text/x-swift">Swift</option>
							    </select>
							</div>
						</div>
						<div class="col-md-12">
...
<form id="modifyCodeForm" method="post">
	<div class="modal" id="updateCodeModal" style="background: rgba(0, 0, 0, 0.5);">
		<div class="modal-dialog modal-dialog-scrollable modal-xl">
...
<!-- ===================================================================================================================================== -->
<!-- ag그리드 크기 -->
<div id="myCodeGird" style="height: 500px; width: 100%;" class="ag-theme-alpine"></div>
...
<script type="text/javascript">
    ...
    <!-- 코드 게시판 입력과 수정을 나누어서 처리, 언어 변환시 에디터 중복 생성을 방지하기 위해 에디터 initialize -->
...
//         let apiKey = "{apiKey}";
	...
	function initializeEditor(language) {
        ...
        let selectElement = document.getElementById("updateLanguageChoice");
        let selectedOption = selectElement.options[selectElement.selectedIndex];
        let selectedMimeType = selectedOption.getAttribute("data-mime-type");
	...
	function changeLanguage(){
		...
		var mode = $(selectedOption).attr('data-mime-type');
...
//                 document.getElementById("code").value = response;
	...
	const modModalOpen = () => {
		...
		$("#updateCodeContent").find("[name=codeTitle]").val($(myCodeModal).find("#codeTitle").val());
...
//                 gridOptions.api.setRowData(JSON.parse(xhr.responseText));	// 덮어쓰는것
    ...
    const gridOptions = {
        ...
        onCellClicked: params => { // example event handler
            ...
            $("#codeTitle").val(codeWallVo.codeTitle);
...
// 				data: generateSendData('json', form),
				...
				success: function(res) {
					...
					console.log("확인 : ", res);
					$('#updateCodeModal').hide();
					$('#modifyCodeForm')[0].reset();
					getCodeData();
				},
				error: function(xhr, status, error) {
					Swal.fire({
