...
//		List<ProjectAttendVO> projectAttend =  memberService.getAllAttendRole(memberVo.getMemberId());
		...
		for(ProjectWorkVO vo : calendarList) {
			...
			LocalDate endDate = vo.getProjectWorkEndDate().plusDays(1);
			map.put("end",endDate+"");
...
//			map.put("borderColor",vo.getBorderColor());
	 ...
	 */
	@ResponseBody
	@PostMapping(value ="calUpdateDrag", produces="application/json;charset=utf-8") //
	public int calUpdateDrag(@RequestBody ProjectWorkVO projectWorkVO,
								@PathVariable int companyId,
								@PathVariable int projectId
			) {
		log.info("@@@@@@@@@수정수정{}", projectWorkVO);
		
		return service.calUpdateDrag(projectWorkVO);
	}
