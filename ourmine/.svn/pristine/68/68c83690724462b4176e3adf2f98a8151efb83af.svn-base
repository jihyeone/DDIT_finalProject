...
public class ProjectIssueServiceImpl implements ProjectIssueService{
	...
	public int insertProjectIssue(ProjectIssueVO projectIssue, Authentication user) {
		...
		log.info("projectIssue={}",projectIssue);
		MemberVOWrapper principal = (MemberVOWrapper) user.getPrincipal();
		projectIssueDAO.alarmReceiverList(projectIssue.getProjectId());
		alarms.forEach(item ->{
			...
			item.setSenderId(projectIssue.getMemberId());
			
			String memberId =  String.valueOf(principal.getRealUser().getMemberId());
			AlarmSendUtil.send(principal.getRealUser().getMemberName(), memberId, projectIssue.getIssueContent(), null);
		});
