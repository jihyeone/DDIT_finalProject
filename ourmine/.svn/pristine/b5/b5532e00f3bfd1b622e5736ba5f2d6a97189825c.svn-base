...
public class SampleEchoWebSocketHandler extends TextWebSocketHandler {
	...
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
        ...
        if (StringUtils.equalsIgnoreCase("roomConnect", roomConnect)) {
    		...
    		log.debug("mapmap : {}", map);
	   		 
    		Authentication auth = (Authentication) session.getPrincipal();
	 		
	        MemberVOWrapper memberVOWrapper = (MemberVOWrapper) auth.getPrincipal();
	        String memberName = memberVOWrapper.getRealUser().getMemberName();
    		String senderNicName = memberName;
    		
    		MessageVO messageVO = new MessageVO(sender, realMsg,senderNicName);
