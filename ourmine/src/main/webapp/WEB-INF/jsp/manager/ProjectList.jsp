...
<form class="editForm">
    <div class="modal fade" id="editproject2" tabindex="-1" aria-hidden="true">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        ...
                        <input type="text" class="projectNameIn form-control" id="exampleFormControlInput78" >
    ...
    <div class="modal fade" id="deleteproject2" tabindex="-1" aria-hidden="true">
        ...
        <div class="modal-dialog modal-dialog-centered modal-md modal-dialog-scrollable">
            <div class="modal-content">
                ...
                <div class="modal-header">
                    <h5 class="modal-title  fw-bold" id="deleteprojectLabel"> 프로젝트를 삭제하시겠습니까?</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <button type="button" class="btn-close memberModalClose" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
...
<form class="insertProjectForm" method="post">
    <div class="modal fade" id="createproject" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-md modal-dialog-scrollable">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title  fw-bold" id="createprojectlLabel"> 프로젝트 생성</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>

                <div class="modal-body">
                    <div class="mb-3">
                        <label for="exampleFormControlInput77" class="form-label">프로젝트 이름</label>
                        <input type="text" class="form-control" id="exampleFormControlInput77" placeholder="프로젝트 이름">
                    ...
                    <div class="deadline-form">
                        ...
                        <div class="row g-3 mb-3">
                            <div class="col">
                                <label for="datepickerded" class="form-label">프로젝트 시작날짜</label>
                                <input type="date" class="form-control" id="datepickerded">
                            </div>
                            <div class="col">
                                <label for="datepickerdedone" class="form-label">프로젝트 종료날짜</label>
                        ...
                        <div class="deadline-form">
                            <div class="row g-3 mb-3">
                                ...
                                <div class="col">
                                    <label class="form-label">PL 지정</label>
                    ...
                    <div class="mb-3">
                        <label for="exampleFormControlTextarea78" class="form-label">프로젝트 내용</label>
            ...
            .then((data,i) => {
                ...
                plList.forEach(list =>{
                    list.innerHTML += data.map(item =>
                        ` <option data-optionid=\${item.memberId} data-optionname=\${item.memberName} value="\${item.memberId}">\${item.memberName}</option> `
                    )
                })
            })
    }
     ...
     <div class="cardList col-xxl-4 col-xl-4 col-lg-4 col-md-6 col-sm-6" data-ccc =\${item.companyId} data-proId = \${item.projectId} data-proname ="\${item.projectName}" data-procontent ="\${item.projectContent}" data-prostartday =\${item.projectStartDate} data-proendday=\${item.projectEndDate}  style="width:400px;float:left;margin-right: 5px;margin-top:5px;">
	     ...
	     <div class="d-flex align-items-center justify-content-between mt-3">
	         ...
	         <div class="btn-group" role="group" aria-label="Basic outlined example">
	         	    ...
	         	    <button type="button" class="btn btn-outline-secondary" data-bs-toggle="modal" data-bs-target="#editproject2" onclick="editPname(\${item})"><i class="icofont-edit text-success"></i></button>
	         	   
		             <button type="button" class="btn btn-outline-secondary" data-bs-toggle="modal" data-bs-target="#deleteproject2"><i class="icofont-ui-delete text-danger"></i></button>
	         </div>
	     </div>
        ...
        document.getElementById("startedList").addEventListener("click", function () {
            allProjects.forEach((item, i) => {
                ...
                if (data !== 'STARTED') {
                    ...
                    console.log("blockprojList[i]", projList[i]);
                }
            })
        })
...
//             let projname = $(".projectNameHidden").val();
//             let projcont = $(".projectContentHidden").val();
            let projname = $(this).data("proname");
        ...
        // ==================== 프로젝트 삭제 ====================
        let delForm = document.querySelector('.delForm2');
    ...
    //=========================프로젝트 생성=========================
        ...
        plList.forEach((item,i) =>(
            item.addEventListener('change', function (e){
                ...
                let selectname = option.getAttribute("data-optionname");
                console.log("select", select);
                console.log("selectname", selectname);
                if(i === 0){
                    insertdata.projectMembers[0].memberId = select;
                    insertdata.projectMembers[0].projectRoleId = 1;
                    insertdata.projectAlarms[0].receiverId = select;
                 	insertdata.projectAlarms[0].alarmReceiver = selectname;
                }else{
                    insertdata.projectMembers[1].memberId = select;
                    insertdata.projectMembers[1].projectRoleId = 2;
                    insertdata.projectAlarms[1].receiverId = select;
                 	insertdata.projectAlarms[1].alarmReceiver = selectname;
                }
...
//         insertdata.projectAlarms[0].senderId
//         insertdata.projectAlarms[0].receiverId
//         insertdata.projectAlarms[0].alarmReceiver
 ...
 function searchMembers() {
	...
	$('#autoBtn').on('click',function(){
		console.log("자동완성클릭");
	$("#exampleFormControlInput77").val("전자정부 공공정보 시스템 구축");
	document.getElementById("datepickerded").value = getCurrentDate();
   document.getElementById("datepickerdedone").value = getCurrentDate();
	$("#exampleFormControlTextarea78").val("공공기관의 업무 효율과 국민 서비스 품질을 향상을 목표로 합니다.");
	$("#datepickerded").val()
...
<script>
document.querySelectorAll(".memberModalClose").forEach(item => {
    item.addEventListener('click', () => {
    	$('#addUser').modal('hide');
    	console.log("멤버추가모달 닫힘");
    	$("#proList").empty();
    	projectList();
    })
