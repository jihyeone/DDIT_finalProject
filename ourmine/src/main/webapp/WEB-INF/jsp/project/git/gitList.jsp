...
<script type="text/javascript">
	const autoToken = () => {
		...
		tokenInput.value = "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";
	}
</script>
	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-footer">
                <button type="button" class="btn" style="color:gray;" onclick="autoToken()">자동완성</button>
...
async function branchStatus(){
	await axios.get(`/ourmine/company/\${companyIdHiddenValue}/project/\${projectIdHiddenValue}/git/gitCommitCount`, {headers : headers})
	  ...
	  function createListMember(item) {
		  let content = '';
		 item.map(user => {
			 let image = '${cPath}/resources/images/lg/마스코트_투명.png';
			 content +=   `<li class="list-group-item px-md-4 py-3 py-md-4">
				<a href="javascript:void(0);" class="d-flex">
					<img class="avatar rounded-circle" src="\${user.imageContents !== null ? user.imageContents : image}" alt="">
					<div class="flex-fill ms-3 text-truncate">
	  ...
	  function drawChart() {
		  return new Promise((resolve, reject) => {
			  $(document).ready(function() {
				  try {
					  let colors = ['var(--chart-color1)', 'var(--chart-color2)', 'var(--chart-color3)',  'var(--chart-color5)'];
